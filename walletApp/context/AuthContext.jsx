...
export const AuthContextProvider = ({children}) => {
    ...
    const logInWithMetamask = async (walletAddress, uuid, signature) => {
        try {
            setIsLoading(true);
            const resp = await Api.post("/auth/authentication", {
            ...
            Cookies.set("token_expiry", resp.data.tokenExpiryTime);
            Cookies.set("isAuthenticated", true);
            window.localStorage.setItem("firstUser", true);
            router.push("/dashboard");
            return Promise.resolve(true);
        } catch (error) {
            toast({
    ...
    const updateUser = async (data) => {
        try {
            ...
            if (resp.data.data) {
                ...
                Cookies.set("user", JSON.stringify(resp.data.data));
